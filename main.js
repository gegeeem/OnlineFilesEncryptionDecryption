...
getExtdDataForEnc.addEventListener("change", () => {
	// nakon sto su ucitani postojeci podaci za enc, prikazi dugem "Pokreni enkripciju"
	const extension = getExtdDataForEnc.files[0].type.slice(
		getExtdDataForEnc.files[0].type.indexOf("/") + 1
	); // npr image/png extension = png
...
getCheckBoxForUploadingData.addEventListener(
	"click",
	handleGetChechBoxForUploadingData
);
...
function handleRstBtnForDec() {
	...
	const getUploadedFileForDecryption =
		document.querySelector("#textForDecFile");
