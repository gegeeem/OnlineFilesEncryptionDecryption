...
export function encryption() {
	...
	reader.onload = async () => {
		// nakon zavrsetka ocitavanja fajla
		...
		ciphertext = await window.crypto.subtle
			// enkripcija uploadovanog fajla
			.encrypt(
				{
					name: "AES-GCM",
					iv: iv,
			...
			.then((res) => {
				...
				wrapCryptoKey(key).then((res) => {
					...
					createFileForDownload(
						"#dataFromEncryption",
						"data:application/json;utf8," + JSON.stringify(DataForDecryption),
						"podaci za dekripciju"
					);

					showHide("show", "#nakonEnkripcije");
					showHide("hide", ".userKeyIVLabel");
					const getDataEncryptionHeader =
						document.querySelector("#encFileHeader");
			...
			.catch((e) => {
				alert(
					"Greška! Netačni parametri za enkripciju ili greška u aplikaciji."
				);
			});
	};
}
